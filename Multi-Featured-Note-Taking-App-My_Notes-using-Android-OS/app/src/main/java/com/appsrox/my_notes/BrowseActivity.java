...
import com.appsrox.my_notes.note.BasicActivity;
import com.appsrox.my_notes.note.ChecklistActivity;
import com.appsrox.my_notes.note.SnapshotActivity;
...
public class BrowseActivity extends Activity implements AdapterView.OnItemClickListener {
	...
	protected void onResume() {
		...
		Cursor c = Note.list(db, null, getOrderBy(my_notes.getDefaultSort()));
		startManagingCursor(c);
		SimpleCursorAdapter adapter = new SimpleCursorAdapter(
				...
				new String[]{Note.COL_TITLE, 
							Note.COL_TYPE, 
							my_notes.getTimeOption()==0 ? Note.COL_MODIFIEDTIME : Note.COL_CREATEDTIME}, 
	...
	static void openNote(long id, Context ctx) {
    	...
    	if (!isLocked) {
	    	...
	    	category.load(my_notes.db);
	    	isLocked = category.isLocked();
    	}
    	
    	Intent intent = new Intent();
        // authenticate
    	if (isLocked && !my_notes.isAuth()) {
