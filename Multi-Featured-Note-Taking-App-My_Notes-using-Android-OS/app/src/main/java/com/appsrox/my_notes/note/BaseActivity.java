...
abstract class BaseActivity extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        spinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
			public void onItemSelected(AdapterView<?> parent, View view, int position, long id) {
				...
				my_notes.LASTSELECTED_CATEGORYID = id;
			}
	...
	private long getDefaultCategoryId() {
		...
		switch(my_notes.getDefaultCategoryOpt()) {
		case 1:
			for (int i=0; i<count; i++) {
				if (adapter.getItemId(i) == my_notes.LASTCREATED_CATEGORYID) {
					return my_notes.LASTCREATED_CATEGORYID;
				}			
			}			
			break;
		case 2:
			for (int i=0; i<count; i++) {
				if (adapter.getItemId(i) == my_notes.LASTSELECTED_CATEGORYID) {
					return my_notes.LASTSELECTED_CATEGORYID;
				}			
			}			
			break;			
		}

		return my_notes.PUBLIC_CATEGORYID;
	}
